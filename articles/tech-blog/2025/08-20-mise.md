...
publishedAt: '2025-08-21T08:28:59+09:00'
topics: [バージョン管理ツール, mise, Node.js, Tips]
dependencies: 'mise'
...
miseのバージョン「2025.10.0」以降から、`.nvmrc`や`.node-version`といった慣用的なバージョンファイルの自動読み込みが無効になる[^1]ため、明示的にどうしたいかを示す必要があります。
...
実際に新しいPCにmiseをインストールした際には、以下のような警告が出ました。
...
設定ファイルが存在していないときは、作った後でmiseに信頼できる設定ファイルだということを伝える必要があります[^3]。
