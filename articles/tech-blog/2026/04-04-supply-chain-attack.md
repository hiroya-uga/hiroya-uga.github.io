...
Renovateではアップデートの性質によってエイジゲートの日数を調整できるので、通常は3日、差分が大きいものについては5日、セキュリティパッチについては即時（`vulnerabilityAlerts.minimumReleaseAge: "0 days"`）に設定しておくのが良さそうです。こうすることで、緊急度の高いセキュリティパッチ[^5]は即座にPRが作成され、それ以外は指定した日数待機してからPRが作成されます。

[^5]: 緊急度の高いセキュリティパッチかどうかは[GitHub Security Alerts](https://docs.github.com/ja/code-security/concepts/security-at-scale/auditing-security-alerts)や[Open Source Vulnerabilities](https://osv.dev/)などの脆弱性データベースを参照して判別しているらしい。[CVE](https://www.ipa.go.jp/security/vuln/scap/cve.html)があるかどうかが判断基準にみえる。
...
- `min-release-age` を使って公開直後のバージョンのリスクを回避する
- ロックファイルを厳格に管理し、意図しない変更を防ぐ
- `ignore-scripts` でインストール時のコード実行をブロックする
- アップデートツールに待機期間を設けて二重の防御を実現する

小さな設定変更ですが、サプライチェーンを盲目的に信頼しないゼロトラストな開発環境へ向けた、意義ある第一歩です。

みなさんのプロジェクトもぜひ見直してみてください 💡

ところでこの`min-release-age`の概念って日本語でなんていうのがいいんだろう。クールダウン？エイジゲート？
