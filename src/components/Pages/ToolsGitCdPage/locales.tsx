...
export const gitCdLocales = {
  ja: {
    introText: (
      <>
        <code>git cd</code> コマンドでコンピューター
        内のローカルリポジトリを検索し、選択したリポジトリのディレクトリに移動できます。
      </>
    ),
    ...
    commandGroups: {
      options: {
        ...
        features: [
          ...
          { cmd: '--submodules', desc: 'git サブモジュールも一覧に含める（デフォルトでは除外）' },
          {
            cmd: '--cache',
    ...
    fzfNote: (
      <>
        <FzfExternalLink />
        がインストール済みの場合はキーワードで絞り込みながら選択でき、未インストールの場合は番号付きリストから選択します。
      </>
    ),
    platforms: {
      macOS: {
  ...
  en: {
    ...
    commandGroups: {
      options: {
        ...
        features: [
          ...
          { cmd: '--submodules', desc: 'Include git submodules in the list (excluded by default)' },
          {
            cmd: '--cache',
    ...
    fzfNote: (
      <>
        If <FzfExternalLink /> is installed, you can filter and select repositories by typing keywords. Otherwise, a
