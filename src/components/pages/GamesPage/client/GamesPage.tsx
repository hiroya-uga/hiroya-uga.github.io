...
export const GamesPage = ({ metadata }: Readonly<Props>) => {
  const id = useId();
  ...
  return (
    <>
      ...
      <dl className="w640:grid-cols-2 w1024:grid-cols-3 grid grid-cols-1 gap-6">
          ...
          .map(({ key, pageTitle, pathname, description }) => {
            ...
            return (
              <ClickableArea
                key={key}
                as="div"
                className="bg-secondary w640:p-4 w1024:gap-4 w1024:p-6 group relative flex flex-col items-start gap-2 rounded-lg px-3 py-2 pr-12 shadow-md transition-shadow duration-300 hover:shadow-lg"
                defaultClickable={linkAreaId}
              >
                <dt className="text-balance">
